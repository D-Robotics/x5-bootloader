...
class SparseImage(object):
    ...
    the mapping between files and their blocks. clobbered_blocks contains the
    set of blocks that should be always written to the target regardless of
    the old contents (i.e. copying instead of patching). clobbered_blocks
    should be in the form of a string like "0" or "0 1-5 8".
    """
    ...
    def __init__(self, simg_fn, file_map_fn=None, clobbered_blocks=None,
        ...
        if major_version != 1 or minor_version != 0:
            raise ValueError("I know about version 1.0, but this is version "
                             "%u.%u" % (major_version, minor_version))
        if file_hdr_sz != 28:
            raise ValueError("File header size was expected to be 28, but is "
                             "%u." % (file_hdr_sz,))
        if chunk_hdr_sz != 12:
            raise ValueError("Chunk header size was expected to be 12, but is "
                             "%u." % (chunk_hdr_sz,))
        ...
        if not build_map:
            assert not hashtree_info_generator, \
                "Cannot generate the hashtree info without building the "\
                "offset map."
            return
        ...
        for i in range(total_chunks):
            ...
            if chunk_type == 0xCAC1:
                if data_sz != (chunk_sz * blk_sz):
                    raise ValueError(
                        "Raw chunk input size (%u) does not match output size "
                        "(%u)" % (data_sz, chunk_sz * blk_sz))
                else:
                ...
                if hashtree_info_generator:
                    fill_data = '\x00' * 4
                    # In order to compute verity hashtree on device, we need
                    # to write zeros explicitly to the don't care ranges.
                    # Because these ranges may contain non-zero data from
                    # the previous build.
                    care_data.append(pos)
                    care_data.append(pos + chunk_sz)
                    offset_map.append((pos, chunk_sz, None, fill_data))
        ...
        # read-ahead. It will fail if such blocks contain non-zeroes.
        # We zero out the extended blocks explicitly to avoid dm-verity
        # failures. 512 blocks are the maximum read-ahead we configure
        # for dm-verity block devices.
    ...
    def _GetRangeData(self, ranges):
        ...
        with self.generator_lock:
            for s, e in ranges:
                ...
                chunk_start, chunk_len, filepos, fill_data = \
                    self.offset_map[idx]
                ...
                while to_read > 0:
                    # continue with following chunks if this range spans
                    # multiple chunks.
                    idx += 1
                    chunk_start, chunk_len, filepos, fill_data = \
                        self.offset_map[idx]
    ...
    def LoadFileBlockMap(self, fn, clobbered_blocks, allow_shared_blocks):
        ...
        with open(fn) as f:
            for line in f:
                ...
                # Note: e2fsdroid records holes in the extent tree as "0"
                # blocks. This causes confusion because clobbered_blocks
                # always includes the superblock (physical block #0).
                # Since the 0 blocks here do not represent actual physical
                # blocks, remove them from the set.
                ranges = raw_ranges.subtract(rangelib.RangeSet("0"))
                # b/150334561 we need to perserve the monotonic property of
                # the raw range. Otherwise, the validation script will read
                # the blocks with wrong order when pulling files from the
                # image.
                ...
                if allow_shared_blocks:
                    ...
                    # If so, tag the entry so that we can skip applying imgdiff
                    # on this file.
                    ...
                    if shared_blocks:
                        ...
                        # Put the non-shared RangeSet as the value in the
                        # block map, which has a copy of the original RangeSet.
                ...
                # Currently we assume that blocks in clobbered_blocks are not
                # part of any file.
        ...
        # aren't part of the data for any file nor part of the
        # clobbered_blocks), divide them into blocks that are all zero
        # and blocks that aren't. (Zero blocks are handled specially
        # because (1) there are usually a lot of them and (2) bsdiff
        # handles files with long sequences of repeated bytes especially
        # poorly.)
        ...
        # Workaround for bug 23227672. For squashfs, we don't have a
        # system.map. So the whole system image will be treated as a single
        # file. But for some unknown bug, the updater will be killed due to
        # OOM when writing back the patched image to flash (observed on
        # lenok-userdebug MEA49). Prior to getting a real fix, we evenly
        # divide the non-zero blocks into smaller groups (currently 1024
