...
class Options(object):
    ...
    def __init__(self):
        # Set up search path, in order to find framework/ and lib64/. At the
        # time of running this function, user-supplied search path (`--path`)
        # hasn't been available. So the value set here is the default, which
        # might be overridden by commandline flag later.
        exec_path = os.path.realpath(sys.argv[0])
        if exec_path.endswith('.py'):
            script_name = os.path.basename(exec_path)
            # logger hasn't been initialized yet at this point. Use print to
            # output warnings.
            print(
                'Warning: releasetools script should be invoked as hermetic '
                'Python executable -- build and run `{}` directly.'.format(
                    script_name[:-3]),
                file=sys.stderr)
...
def InitLogging():
    DEFAULT_LOGGING_CONFIG = {
        ...
        'formatters': {
            'standard': {
                'format':
                    '%(asctime)s - %(filename)s - %(levelname)-8s: '
                    '%(message)s',
                'datefmt': '%Y-%m-%d %H:%M:%S',
            },
        },
...
def Run(args, verbose=None, **kwargs):
    ...
    Args:
      ...
      kwargs: Any additional args to be passed to subprocess.Popen(), such as
          env, stdin, etc. stdout and stderr will default to subprocess.PIPE
          and subprocess.STDOUT respectively unless caller specifies any of
          them. universal_newlines will default to True, as most of the users
          in releasetools expect string output.
    ...
    if args:
        # Make a copy of args in case client relies on the content of
        # args later.
        args = args[:]
...
def RunAndCheckOutput(args, verbose=None, **kwargs):
    ...
    Args:
      ...
      kwargs: Any additional args to be passed to subprocess.Popen(), such as
          env, stdin, etc. stdout and stderr will default to subprocess.PIPE
          and subprocess.STDOUT respectively unless caller specifies
          any of them.
...
class BuildInfo(object):
    ...
    This class wraps up the property querying for a given source or target
    build. It abstracts away the logic of handling OEM-specific properties,
    and caches the commonly used properties such as fingerprint.

    There are two types of info dicts: a) build-time info dict, which is
    generated at build time (i.e. included in a target_files zip); b)
    OEM info dict that is specified at package generation time (via command
    line argument '--oem_settings'). If a build doesn't use OEM-specific
    properties (i.e. not having "oem_fingerprint_properties" in build-time
    info dict), all the queries would be answered based on build-time info
    dict only. Otherwise if using OEM-specific properties, some of them will
    be calculated from two info dicts.

    Users can query properties similarly as using a dict()
    (e.g. info['fstab']), or to query build properties via GetBuildProp()
    or GetPartitionBuildProp().

    Attributes:
      info_dict: The build-time info dict.
      is_ab: Whether it's a build that uses A/B OTA.
      oem_dicts: A list of OEM dicts.
      oem_props: A list of OEM properties that should be read from OEM dicts;
          None if the build doesn't use any OEM-specific property.
      fingerprint: The fingerprint of the build, which would be calculated
          based on OEM properties if applicable.
    ...
    def __init__(self, info_dict, oem_dicts=None, use_legacy_id=False):
        ...
        Arguments:
          info_dict: The build-time info dict.
          oem_dicts: A list of OEM dicts (which is parsed from
              --oem_settings). Note that it always uses the first dict to
              calculate the fingerprint or the device name. The rest would
              be used for asserting OEM properties only (e.g. one package
              can be installed on one of these devices).
          use_legacy_id: Use the legacy build id to construct the fingerprint.
              This is used when we need a BuildInfo class, while the vbmeta
        ...
        def check_fingerprint(fingerprint):
            if (" " in fingerprint or
                    any(ord(ch) > 127 for ch in fingerprint)):
                raise ValueError(
                    'Invalid build fingerprint: "{}". See the requirement in '
                    "Android CDD  3.2.2. Build Parameters.".
                    format(fingerprint))
        ...
        if "system" in self._partition_fingerprints:
            # system_other is not included in PARTITIONS_WITH_BUILD_PROP, but
            # does need a fingerprint when creating the image.
            self._partition_fingerprints[
    ...
    def GetPartitionBuildProp(self, prop, partition):
        ...
        if prop in BuildInfo._RO_PRODUCT_RESOLVE_PROPS:
            prop = prop.replace(
                "ro.product", "ro.product.{}".format(prop_partition))
        else:
    ...
    def GetBuildProp(self, prop):
        """Returns the inquired build property from the standard
            build.prop file."""
        if prop in BuildInfo._RO_PRODUCT_RESOLVE_PROPS:
    ...
    def _GetRoProductPropsDefaultSourceOrder(self):
        # NOTE: refer to CDDs and android.os.Build.VERSION for the definition
        # and values of these properties for each Android release.
        android_codename = self._GetRawBuildProp(
            "ro.build.version.codename", None)
        if android_codename == "REL":
            android_version = self._GetRawBuildProp(
                "ro.build.version.release", None)
            if android_version == "10":
                return BuildInfo.\
                    _RO_PRODUCT_PROPS_DEFAULT_SOURCE_ORDER_ANDROID_10
            # NOTE: float() conversion of android_version will have rounding
            # error. We are checking for "9" or less, and using "< 10" is well
            # outside of possible floating point rounding.
            try:
    ...
    def _GetPlatformVersion(self):
        ...
        # init code switches to version_release_or_codename (see b/158483506).
        # After API finalization, release_or_codename will be the same as
        # release. This is the best effort to support pre-S dev stage builds.
        if int(version_sdk) >= 30:
            try:
                return self.GetBuildProp("ro.build.version."
                                         "release_or_codename")
            except ExternalError:
                logger.warning(
                    'Failed to find ro.build.version.release_or_codename')
    ...
    def _GetBuildId(self):
        ...
        # logic needs to match the one in init, so that OTA can deliver
        # correctly.
        ...
        if not vbmeta_digest:
            raise ExternalError("Vbmeta digest isn't provided when using "
                                "legacy build id")
    ...
    def _GetPartitionPlatformVersion(self, partition):
        try:
            return self.GetPartitionBuildProp("ro.build.version."
                                              "release_or_codename",
                                              partition)
        except ExternalError:
    ...
    def CalculatePartitionFingerprint(self, partition):
        try:
            return self.GetPartitionBuildProp("ro.build.fingerprint",
                                              partition)
        except ExternalError:
            return "{}/{}/{}:{}/{}/{}:{}/{}".format(
...
def ExtractFromInputFile(input_file, fn):
    """Extracts the contents of fn from input zipfile or directory
        into a file."""
    if isinstance(input_file, zipfile.ZipFile):
...
def LoadInfoDict(input_file, repacking=False):
    ...
    It reads `META/misc_info.txt` file in the target_files input, does
    validation checks and returns the parsed key/value pairs for to the
    given build. It's usually called early when working on input target_files
    files, e.g. when generating OTAs, or signing builds. Note that the function
    may be called against an old target_files file (i.e. from past dessert
    releases). So the property parsing needs to be backward compatible.

    In a `META/misc_info.txt`, a few properties are stored as links to the
    files in the PRODUCT_OUT directory. It works fine with the build system.
    However, they are no longer available when (re)generating images from
    target_files zip. When `repacking` is True, redirect these properties
    to the actual files in the unzipped directory.

    Args:
      input_file: The input target_files file, which could be an open
          zipfile.ZipFile instance, or a str for the dir that contains the
          files unzipped from a target_files file.
      repacking: Whether it's trying repack an target_files file after loading
          the info dict (default: False). If so, it will rewrite a few loaded
          properties (e.g. selinux_fc, root_dir) to point to the actual files
          in target_files file. When doing repacking, `input_file` must be
          a dir.
    ...
    try:
        ...
        raise ValueError(
            "Failed to find META/misc_info.txt in input target-files")
    ...
    if repacking:
        ...
        for part_name in ["system", "vendor", "system_ext", "product", "odm",
            ...
            if os.path.exists(base_fs_file):
                ...
                logger.warning(
                    "Failed to find %s base fs file: %s", part_name,
                    base_fs_file)
                del d[key_name]
...
class PartitionBuildProps(object):
    ...
    def _ReadBootPropFile(input_file, ramdisk_format):
        ...
        try:
            ...
            return ''
        prop_file = GetBootImageBuildProp(
            boot_img, ramdisk_format=ramdisk_format)
    ...
    def _LoadBuildProp(self, data):
        for line in data.split('\n'):
            ...
            if line.startswith("import"):
                ...
                if duplicates:
                    raise ValueError('prop {} is overridden multiple times'.
                                     format(','.join(duplicates)))
                self.prop_overrides = self.prop_overrides.union(
                    overrides.keys())
                self.build_props.update(overrides)
            elif "=" in line:
                name, value = line.split("=", 1)
                if name in self.prop_overrides:
                    raise ValueError('prop {} is set again after overridden by'
                                     ' import statement'.format(name))
    ...
    def _ImportParser(self, line):
        ...
        # ${placeholder}, and its value should be provided by the caller
        # through the placeholder_values.
...
def LoadRecoveryFSTab(read_helper, fstab_version, recovery_fstab_path,
    ...
    # / is used for the system mount point when the root directory is included
    # in system. Other areas assume system is always at "/system" so point
    # /system at /.
...
def _FindAndLoadRecoveryFstab(info_dict, input_file, read_helper):
    ...
    # We changed recovery.fstab path in Q, from ../RAMDISK/etc/recovery.fstab
    # to ../RAMDISK/system/etc/recovery.fstab. This function has to handle
    # both cases, since it may load the info_dict from an old build
    # (e.g. when generating incremental OTAs from that build).
...
def MergeDynamicPartitionInfoDicts(framework_dict, vendor_dict):
    ...
    Args:
      framework_dict: The dictionary of dynamic partition info variables from
        the partial framework target files.
    ...
    if (framework_dict.get("use_dynamic_partitions") !=
        ...
        raise ValueError(
            "Both dictionaries must have use_dynamic_partitions=true")
    ...
    if "super_block_devices" in vendor_dict:
        ...
        for block_device in merged_dict["super_block_devices"].split(" "):
            ...
            if key not in vendor_dict:
                raise ValueError(
                    "Vendor dict does not contain required key %s." % key)
            merged_dict[key] = vendor_dict[key]

    # Partition groups and group sizes are defined by the vendor dict because
    # these values may vary for each board that uses a shared system image.
    merged_dict["super_partition_groups"] = \
        vendor_dict["super_partition_groups"]
...
def SharedUidPartitionViolations(uid_dict, partition_groups):
    ...
    Args:
      uid_dict: A dictionary created by using the standard json module to read
        a complete shareduid_violation_modules.json file.
    ...
    for uid, partitions in uid_dict.items():
        ...
        if len(found_in_groups) > 1:
            errors.append(
                "APK sharedUserId \"%s\" found across partition groups in "
                "partitions \"%s\""
...
def RunHostInitVerifier(product_out, partition_map):
    ...
    for partition, path in partition_map.items():
        if partition not in allowed_partitions:
            raise ExternalError("Unable to call host_init_verifier for "
        ...
        property_contexts_path = os.path.join(product_out, path, "etc",
                                              "selinux",
                                              property_contexts)
...
def GetAvbPartitionArg(partition, image, info_dict=None):
    ...
    It sets up the VBMeta argument by including the partition descriptor from
    the given 'image', or by configuring the partition as a chained partition.
    ...
    # boot chain, to be verified with
...
def GetAvbChainedPartitionArg(partition, info_dict, key=None):
    ...
    Args:
      ...
      info_dict: The info dict to look up the key info and rollback index
          location.
      key: The key to be used for building or verifying the partition.
          Defaults to the key listed in info_dict.

    Returns:
      A string of form "partition:rollback_index_location:key" that can be
      used to build or verify vbmeta image.
    """
    if key is None:
...
def ConstructAftlMakeImageCommands(output_image):
    ...
    version_incremental = build_info.GetBuildProp(
...
def BuildVBMeta(image_path, partitions, name, needed_partitions):
    ...
    Args:
      ...
      needed_partitions: Partitions whose descriptors should be included into
    ...
    if args and args.strip():
        ...
        for index, arg in enumerate(split_args[:-1]):
            ...
            # as a path relative to source tree, which may not be available at
            # the same location when running this script (we have the input
            # target_files zip only). For such cases, we additionally scan
            # other locations (e.g. IMAGES/, RADIO/, etc) before bailing out.
            if arg == '--include_descriptors_from_image':
                ...
                for dir_name in ['IMAGES', 'RADIO', 'PREBUILT_IMAGES']:
                    alt_path = os.path.join(
                        OPTIONS.input_tmp, dir_name,
                        os.path.basename(chained_image))
...
def _MakeRamdisk(sourcedir, fs_config_file=None,
    ...
    assert p2.returncode == 0, "compression of %s ramdisk failed" % (
        sourcedir,)
...
def _BuildBootableImage(image_name, sourcedir, fs_config_file, info_dict=None,
    ...
    Take a kernel, cmdline, and optionally a ramdisk directory from the input
    (in 'sourcedir'), and turn them into a boot image. 'two_step_image'
    indicates if we are building a two-step special image (i.e. building
    a recovery image to be loaded into /boot in two-step OTAs).

    Return the image data, or None if sourcedir does not appear to contains
    files for building the requested image.
    """
    ...
    if has_ramdisk and not os.access(os.path.join(sourcedir, "RAMDISK"),
                                     os.F_OK):
        return None
    ...
    if (info_dict.get("boot_signer") == "true" and
        ...
        # Hard-code the path as "/boot" for two-step special recovery image (
        # which will be loaded into /boot during the two-step OTA).
        if two_step_image:
            path = "/boot"
        else:
            path = "/" + partition_name
    ...
    # Sign the image if vboot is non-empty.
        ...
        # (futility-host) built from the source. Override the setting in the
        # old TF.zip.
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
    ...
    otherwise look for it under 'unpack_dir'/IMAGES, otherwise construct it
    from the source files in 'unpack_dir'/'tree_subdir'."""
    ...
    if os.path.exists(prebuilt_path):
        logger.info(
            "Re-signing prebuilt %s from PREBUILT_IMAGES...", prebuilt_name)
        signed_img = MakeTempFile()
        shutil.copy(prebuilt_path, signed_img)
        partition_name = tree_subdir.lower()
        _SignBootableImage(signed_img, prebuilt_name,
                           partition_name, info_dict)
        return File.FromLocalFile(name, signed_img)

    logger.info("building image from target_files %s...", tree_subdir)

    # With system_root_image == "true", we don't pack ramdisk into the boot
    # image. Unless "recovery_as_boot" is specified, in which case we carry
    # the ramdisk for recovery.
    has_ramdisk = (info_dict.get("system_root_image") != "true" or
                   prebuilt_name != "boot.img" or
                   info_dict.get("recovery_as_boot") == "true")

    fs_config = "META/" + tree_subdir.lower() + "_filesystem_config.txt"
    data = _BuildBootableImage(prebuilt_name, os.path.join(unpack_dir,
                                                           tree_subdir),
...
def _BuildVendorBootImage(sourcedir, info_dict=None):
    ...
    Take a ramdisk, dtb, and vendor_cmdline from the input (in 'sourcedir'),
    and turn them into a vendor boot image.
    ...
    if os.access(fn, os.F_OK):
        ...
        for ramdisk_fragment in ramdisk_fragments:
            ...
            # Use prebuilt image if found, else create ramdisk from supplied
            # files.
            if os.access(fn, os.F_OK):
                ...
                ramdisk_fragment_img = \
                    _MakeRamdisk(ramdisk_fragment_root,
                                 ramdisk_format=ramdisk_format)
                ramdisk_fragment_imgs.append(ramdisk_fragment_img)
                ramdisk_fragment_pathname = ramdisk_fragment_img.name
            cmd.extend(["--vendor_ramdisk_fragment",
                       ramdisk_fragment_pathname])
...
def UnzipToDir(filename, dirname, patterns=None):
    ...
    Args:
      ...
      patterns: Files to unzip from the archive. If omitted, will unzip the
          entire archvie. Non-matching patterns will be filtered out. If
          there's no match after the filtering, no file will be unzipped.
    """
...
def UnzipTemp(filename, pattern=None):
    """Unzips the given archive into a temporary directory and returns
        the name.

    Args:
      filename: If filename is of the form "foo.zip+bar.zip", unzip foo.zip
      into a temp dir, then unzip bar.zip into that_dir/BOOTABLE_IMAGES.

      pattern: Files to unzip from the archive. If omitted, will unzip the
      entire archvie.
...
def GetUserImage(which, tmpdir, input_zip,
    ...
    This function loads the specified image from the given path. If the
    specified image is sparse, it also performs additional processing for
    OTA purpose. For example, it always adds block 0 to clobbered blocks list.
    It also detects files that cannot be reconstructed from the block list,
    for whom we should avoid applying imgdiff.
    ...
    Args:
      ...
      reset_file_map: If true and image is sparse, reset file map before
          returning the image.
    Returns:
    ...
    # When target uses 'BOARD_EXT4_SHARE_DUP_BLOCKS := true', images may
    # contain shared blocks (i.e. some blocks will show up in multiple files'
    # block list). We can only allocate such shared blocks to the first
    # "owner", and disable imgdiff for all later occurrences.
...
def GetNonSparseImage(which, tmpdir, hashtree_info_generator=None):
    ...
    Args:
      which: The partition name.
      tmpdir: The directory that contains the prebuilt image and block map
              file.
    Returns:
      A Image object.
    """
...
def GetSparseImage(which, tmpdir, input_zip, allow_shared_blocks,
    ...
    reconstructed from the block list, for whom we should avoid applying
    imgdiff.
    ...
    # In ext4 filesystems, block 0 might be changed even being mounted R/O. We
    # add it to clobbered_blocks so that it will be written to the target
    # unconditionally. Note that they are still part of care_map.
    # (Bug: 20939131)
    clobbered_blocks = "0"

    image = sparse_img.SparseImage(
        path, mappath, clobbered_blocks,
    ...
    # its block list. Tag such entries accordingly. (Bug: 65213616)
    for entry in image.file_map:
        # Skip artificial names, such as "__ZERO", "__NONZERO-1".
        if not entry.startswith('/'):
            continue

        # "/system/framework/am.jar" => "SYSTEM/framework/am.jar". Note that
        # the filename listed in system.map may contain an additional leading
        # slash (i.e. "//system/framework/am.jar"). Using lstrip to get
        # consistent results.
        ...
        # If a RangeSet has been tagged as using shared blocks while loading
        # the image, check the original block list to determine its
        # completeness. Note that the 'incomplete' flag would be tagged to
        # the original RangeSet only.
    ...
    for k in sorted(keylist):
        ...
        if p.returncode == 0:
            ...
            if p.returncode == 0:
                ...
                # It would have said "Error reading key" if it didn't parse
                # correctly.
...
def GetMinSdkVersion(apk_name):
    ...
    It calls 'aapt2' to query the embedded minSdkVersion from the given APK
    file. This can be both a decimal number (API Level) or a codename.
    ...
    if proc.returncode != 0:
        raise ExternalError(
            ...
            format(proc.returncode, stdoutdata, stderrdata))
...
def GetMinSdkVersionInt(apk_name, codename_to_api_level_map):
    ...
    If minSdkVersion is set to a codename, it is translated to a number using
    the provided map.
...
def SignFile(input_name, output_name, key, password, min_api_level=None,
    ...
    min_api_level is the API Level (int) of the oldest platform this file may
    end up on. If not specified for an APK, the API Level is obtained by
    interpreting the minSdkVersion attribute of the APK's AndroidManifest.xml.
...
def CheckSize(data, target, info_dict):
    ...
    size = len(data)
    # target could be 'userdata' or 'cache'. They should follow the non-AVB
    # image path.
    if info_dict.get("avb_enable") == "true" and target in AVB_PARTITIONS:
        ...
        msg = "%s size (%d) is %.2f%% of limit (%d)" % (
            target, size, pct, limit)
        if pct >= 99.0:
...
def ReadApkCerts(tf_zip):
    ...
    Given a target-files ZipFile, parses the META/apkcerts.txt entry and
    returns a tuple with the following elements: (1) a dictionary that maps
    packages to certs (based on the "certificate" and "private_key" attributes
    in the file; (2) a string representing the extension of compressed APKs
    in the target files (e.g ".gz", ".bro").

    Args:
      tf_zip: The input target_files ZipFile (already open).

    Returns:
      (certmap, ext): certmap is a dictionary that maps packages to certs; ext
          is the extension string of compressed APKs (e.g. ".gz"), or None if
          there's no compressed APKs.
    """
    certmap = {}
    ...
    for line in tf_zip.read('META/apkcerts.txt').decode().split('\n'):
        ...
        m = re.match(
            ...
            r'private_key="(?P<PRIVKEY>.*?)"(\s+compressed=\
            "(?P<COMPRESSED>.*?)")?'
...
Global options
  ...
  -p  (--path) <dir>
      Prepend <dir>/bin to the list of places to search for binaries run by
      this script, and expect to find jars in <dir>/framework.
    ...
    try:
        opts, args = getopt.getopt(
            ...
            ["help", "verbose", "path=", "signapk_path=",
             ...
             "java_path=", "java_args=", "android_jar_path=",
             "public_key_suffix=",
...
def ZipWrite(zip_file, filename, arcname=None, perms=0o644,
    ...
    # limit. Note that `zipfile.writestr()` will not work for strings larger
    # than 2GiB. The Python interpreter sometimes rejects strings that large
    # (though it isn't clear to me exactly what circumstances cause this).
    ...
    try:
        # `zipfile.write()` doesn't allow us to pass ZipInfo, so just modify
        # the file to be zipped and reset it when we're done.
        ...
        # attached, so we need "epoch" but in the local time zone to get
        # 2009/01/01 in the zip archive.
        local_epoch = datetime.datetime.fromtimestamp(0)
        timestamp = (datetime.datetime(2009, 1, 1) -
                     local_epoch).total_seconds()
...
def ZipWriteStr(zip_file, zinfo_or_arcname, data, perms=None,
    ...
    if not isinstance(zinfo_or_arcname, zipfile.ZipInfo):
        ...
        # Python 2 and 3 behave differently when calling ZipFile.writestr()
        # with zinfo.external_attr being 0. Python 3 uses `0o600 << 16` as the
        # value for such a case (since
        # https://github.com/python/cpython/commit/18ee29d0b870caddc0806916ca2c823254f1a1f9),
        # which seems to make more sense. Otherwise the entry will have 0o000
        # as the permission bits. We follow the logic in Python 3 to get
        # consistent behavior between using the two versions.
...
class DeviceSpecificParams(object):
    ...
    def FullOTA_GetBlockDifferences(self):
        ...
        Implementation should return a list of BlockDifference objects
        describing the update on each additional partitions.
        """
        ...
        try:
            ...
            for d in diff_iter:
                ...
                if patch is None:
                    ...
                    logger.info(
                        "%8.2f sec %8d / %8d bytes (%6.2f%%) %s", dur,
                        len(patch),
...
class BlockDifference(object):
    def __init__(self, partition, tgt, src=None, check_first_block=False,
        ...
        is_dynamic = is_dynamic_build and (
            is_dynamic_source or is_dynamic_target)
    ...
    def WriteStrictVerifyScript(self, script):
        ...
        This differs from the WriteVerifyScript() function: a) it prints
        different error messages; b) it doesn't allow half-way updated
        images to pass the verification."""
    ...
    def WriteVerifyScript(self, script, touched_blocks_only=False):
        ...
        # incremental OTA
            ...
            if self.version >= 4:
                ...
                # Bug: 21124327
                # When generating incrementals for the system and vendor
                # partitions in version 4 or newer, explicitly check the first
                # block (which contains the superblock) of the partition to
                # see if it's what we expect. If this check fails, give an
                # explicit log message about the partition having been
                # remounted R/W (the most likely explanation).
                ...
                script.AppendExtra((
                    ...
                    'abort("E{code}: {partition} partition fails to '
                    'recover"));\n endif;').format(device=self.device,
                                                   ranges=ranges_str,
                                                   partition=partition,
                                                   code=code))

            # Abort the OTA update. Note that the incremental OTA cannot be
            # applied even if it may match the checksum of the target
            # partition.
            # a) If version < 3, operations like move and erase will make
            #    changes unconditionally and damage the partition.
    ...
    def WritePostInstallVerifyScript(self, script):
        ...
        # Unlike pre-install verification, clobbered_blocks should
        # not be ignored.
        ranges = self.tgt.care_map
        ranges_str = ranges.to_string_raw()
        script.AppendExtra(
        ...
        if self.tgt.extended:
            ...
            script.AppendExtra(
                'else\n'
                '  abort("E%d: %s partition has unexpected non-zero contents '
                'after OTA update");\n'
                'endif;' % (code, partition))
        else:
            script.Print('Verified the updated %s image.' % (partition,))
    ...
    def _WriteUpdate(self, script, output_zip):
        ...
        # For full OTA, compress the new.dat with brotli with quality 6 to
        # reduce its size. Quailty 9 almost triples the compression time
        # but doesn't further reduce the size too much. For a typical 1.8G
        # system.new.dat
        ...
        call = ('block_image_update({device}, '
                ...
                format(device=self.device, partition=self.partition,
...
def GetTypeAndDeviceExpr(mount_point, info):
    """
    Return the filesystem of the partition, and an edify expression that
    evaluates to the device at runtime.
    """
    fstab = info["fstab"]
    if fstab:
        p = fstab[mount_point]
...
def ExtractPublicKey(cert):
    ...
    """
    # The behavior with '-out' is different between openssl 1.1 and
    # openssl 1.0. While openssl 1.1 writes the key into the given
    # filename followed by '-out', openssl 1.0 (both of 1.0.1 and 1.0.2)
    # doesn't. So we collect the output from stdout instead.
...
def ExtractAvbPublicKey(avbtool, key):
    ...
    Args:
      ...
      key: The input key file, which should be PEM-encoded public or
           private key.
...
def MakeRecoveryPatch(input_dir, output_sink, recovery_img, boot_img,
    ...
    Most of the space in the boot and recovery images is just the kernel,
    which is identical for the two, so the resulting patch should be efficient.
    Add it to the output zip, along with a shell script that is run from
    init.rc on first boot to actually do the patching and install the new
    recovery image.
    ...
    if full_recovery_image:
        ...
        # With system-root-image, boot and recovery images will have
        # mismatching entries (only recovery has the ramdisk entry)
        # (Bug: 72731506). Use bsdiff to handle such a case.
        if system_root_image:
            diff_program = ["bsdiff"]
            bonus_args = ""
    ...
    try:
        ...
        boot_type, boot_device = GetTypeAndDevice("/boot", info_dict,
                                                  check_no_slot=False)
        recovery_type, recovery_device = GetTypeAndDevice("/recovery",
                                                          info_dict,
                                                          check_no_slot=False)
    except KeyError:
        return
    ...
    if full_recovery_image:
        ...
        # Note that we use /vendor to refer to the recovery resources. This
        # will work for a separate vendor partition mounted at /vendor or a
        # /system/vendor subdirectory on the system partition, for which init
        # will create a symlink from /vendor to /system/vendor.
...
""" % {'type': recovery_type,
        ...
        sh = """#!/vendor/bin/sh
if ! applypatch --check %(recovery_type)s:%(recovery_device)s:\
    %(recovery_size)d:%(recovery_sha1)s; then
  applypatch %(bonus_args)s \\
          --patch /vendor/recovery-from-boot.p \\
          --source %(boot_type)s:%(boot_device)s:%(boot_size)d:%(boot_sha1)s \\
          --target %(recovery_type)s:%(recovery_device)s:%(recovery_size)d:\
            %(recovery_sha1)s && \\
      log -t recovery "Installing new recovery image: succeeded" || \\
      log -t recovery "Installing new recovery image: failed"
else
...
""" % {'boot_size': boot_img.size,
    ...
    # The install script location moved from /system/etc to /system/bin in the
    # L release. In the R release it is in VENDOR/bin or SYSTEM/vendor/bin.
...
class DynamicPartitionUpdate(object):
    ...
    def src_size(self):
        if not self.block_difference:
            return 0
        return DynamicPartitionUpdate._GetSparseImageSize(self.
                                                          block_difference.src)
    ...
    def __init__(self, info_dict, block_diffs, progress_dict=None,
        ...
        for g in tgt_groups:
            for p in shlex.split(info_dict.get(
                ...
                assert p in self._partition_updates, \
                    "{} is in target super_{}_partition_list but no " \
                    "BlockDifference object is provided.".format(p, g)
        ...
        for g in src_groups:
            for p in shlex.split(source_info_dict.get(
                    "super_%s_partition_list" % g, "").strip()):
                assert p in self._partition_updates, \
                    "{} is in source super_{}_partition_list but no " \
                    "BlockDifference object is provided.".format(p, g)
                self._partition_updates[p].src_group = g

        target_dynamic_partitions = set(shlex.split(info_dict.get(
            "dynamic_partition_list", "").strip()))
        block_diffs_with_target = set(p for p, u in self._partition_updates.
                                      items() if u.tgt_size)
        assert block_diffs_with_target == target_dynamic_partitions, \
            "Target Dynamic partitions: {}, BlockDifference with target: {}".\
            format(list(target_dynamic_partitions),
                   list(block_diffs_with_target))

        source_dynamic_partitions = set(shlex.split(source_info_dict.get(
            "dynamic_partition_list", "").strip()))
        block_diffs_with_source = set(p for p, u in self._partition_updates.
                                      items() if u.src_size)
        assert block_diffs_with_source == source_dynamic_partitions, \
            "Source Dynamic partitions: {}, BlockDifference with source: {}".\
            format(list(source_dynamic_partitions),
    ...
    def WriteScript(self, script, output_zip, write_verify_script=False):
        ...
        for p, u in self._partition_updates.items():
            if u.src_size and u.tgt_size and u.src_size > u.tgt_size:
                ...
                u.block_difference.WriteScript(script, output_zip,
                                               progress=u.progress,
                                               write_verify_script=False)
        ...
        if write_verify_script:
            for p, u in self._partition_updates.items():
                if u.src_size and u.tgt_size and u.src_size > u.tgt_size:
                    ...
                    script.AppendExtra('unmap_partition("%s");' %
                                       p)  # ignore errors
    ...
    def _Compute(self):
        ...
        for p, u in self._partition_updates.items():
            if u.src_group and u.tgt_group and u.src_group != u.tgt_group:
                comment('Move partition %s from %s to default' %
                        (p, u.src_group))
        ...
        for g, u in self._group_updates.items():
            ...
            if (u.src_size is not None and u.tgt_size is not None and
                ...
                comment('Shrink group %s from %d to %d' %
                        (g, u.src_size, u.tgt_size))
        ...
        for g, u in self._group_updates.items():
            ...
            if (u.src_size is not None and u.tgt_size is not None and
                    u.src_size < u.tgt_size):
                comment('Grow group %s from %d to %d' %
                        (g, u.src_size, u.tgt_size))
                append('resize_group %s %d' % (g, u.tgt_size))
...
def GetBootImageBuildProp(boot_img, ramdisk_format=RamdiskFormat.LZ4):
    ...
    Args:
      boot_img: the boot image file. Ramdisk must be compressed with lz4 or
                minigzip format.
    ...
    try:
        ...
        # Use "toybox cpio" instead of "cpio" because the latter invokes cpio
        # from the host environment.
        RunAndCheckOutput(['toybox', 'cpio', '-F', abs_uncompressed_ramdisk,
                           '-i'], cwd=extracted_ramdisk)
...
def GetBootImageTimestamp(boot_img):
    ...
    Args:
      boot_img: the boot image file. Ramdisk must be compressed with
                lz4 format.
    ...
    try:
        ...
        logger.warning(
            'Unable to get boot image timestamp: ro.bootimage.build.date.utc'
            'is undefined')
        return None
...
def GetCareMap(which, imgname):
    ...
    Returns:
      (which, care_map_ranges): care_map_ranges is the raw string of the
      care_map RangeSet; or None.
    """
    assert which in PARTITIONS_WITH_CARE_MAP

    # which + "_image_size" contains the size that the actual filesystem image
    # resides in, which is all that needs to be verified. The additional
    # blocks in the image file contain verity metadata, by reading which
    # would trigger invalid reads.
    ...
    # For sparse images, we will only check the blocks that are listed in the
    # care map, i.e. the ones with meaningful data.
...
def AddCareMapForAbOta(output_file, ab_partitions, image_paths):
    ...
    for partition in ab_partitions:
        ...
        if (verity_block_device in OPTIONS.info_dict or
            ...
            if partition not in image_paths:
                logger.warning('Potential partition with care_map missing from'
                               ' images: %s', partition)
                continue
            image_path = image_paths[partition]
            if not os.path.exists(image_path):
                raise ExternalError(
                    'Expected image at path {}'.format(image_path))
    ...
    # Converts the list into proto buf message by calling care_map_generator;
    ...
    if care_map_path in output_file.namelist():
        ...
        shutil.copy(temp_care_map, os.path.join(
            OPTIONS.input_tmp, care_map_path))
