...
def BuildVerityMetadata(image_size, verity_metadata_path, root_hash, salt,
    ...
    cmd = ["build_verity_metadata", "build", str(image_size),
           verity_metadata_path, root_hash, salt, block_device, signer_path,
           key]
    if signer_args:
...
def CreateVerityImageBuilder(prop_dict):
    ...
    Args:
      prop_dict: A dict that contains the build properties. In particular,
          it will look for verity-related property values.

    Returns:
      A VerityImageBuilder instance for Verified Boot 1.0 or
...
class VerityImageBuilder(object):
    ...
    the common interface between Verified Boot 1.0 and Verified Boot 2.0. A
    matching builder will be returned based on the given build properties.

    More info on the verity image generation can be found at
    the following link.
    https://source.android.com/security/verifiedboot/dm-verity#implementation
    """

    def CalculateMaxImageSize(self, partition_size):
        """Calculates the filesystem image size for the given
            partition size."""
...
class Version1VerityImageBuilder(VerityImageBuilder):
    ...
    def CalculateDynamicPartitionSize(self, image_size):
        # This needs to be implemented. Note that returning the given image
        # size as the partition size doesn't make sense, as it will fail later.
        raise NotImplementedError
    ...
    def CalculateMaxImageSize(self, partition_size=None):
        ...
        Args:
          partition_size: The partition size, which defaults to
              self.partition_size if unspecified.
        ...
        logger.info(
            "Calculated image size for verity: partition_size %d, image_size "
            "%d, verity_size %d", partition_size, result, verity_size)
        return result
    ...
    def Build(self, out_file):
        ...
        verity_metadata_path = os.path.join(
            tempdir_name, "verity_metadata.img")
        ...
        BuildVerityMetadata(
            image_size, verity_metadata_path, root_hash, salt,
            self.block_device,
...
class VerifiedBootVersion2VerityImageBuilder(VerityImageBuilder):
    ...
    def CalculateMinPartitionSize(self, image_size, size_calculator=None):
        ...
        This is used when determining the partition size for a dynamic
        partition, which should be cover the given image size (for
        filesystem files) as well as the verity metadata size.
        ...
        lo = int(image_size / image_ratio) // BLOCK_SIZE * \
        ...
        while max_image_size < image_size:
            ...
            hi = int(image_size / image_ratio) // BLOCK_SIZE * \
                BLOCK_SIZE + delta
            delta *= 2
            max_image_size = size_calculator(hi)
        ...
        while lo < hi:
            ...
            # if mid can accommodate image_size
            if max_image_size >= image_size:
                # if a smaller partition size is found
                if mid < partition_size:
                    partition_size = mid
                hi = mid
            else:
                lo = mid + BLOCK_SIZE
    ...
    def CalculateMaxImageSize(self, partition_size=None):
        ...
        add_footer = ("add_hash_footer" if self.footer_type ==
                      self.AVB_HASH_FOOTER
    ...
    def Build(self, out_file):
        ...
        if proc.returncode != 0:
            raise BuildVerityImageError(
                "Failed to add AVB footer: {}".format(output))
...
class VerifiedBootVersion1HashtreeInfoGenerator(HashtreeInfoGenerator):
    ...
    def __init__(self, partition_size, block_size, fec_supported):
        ...
        Arguments:
          ...
          block_size: Expected size in bytes of each block for the sparse
        ...
        prop_dict = {
            ...
            # 'verity_block_device' needs to be present to indicate a
            # verity-enabled partition.
            'verity_block_device': '',
            # We don't need the following properties that are needed for
            # signing the verity metadata.
            'verity_key': '',
    ...
    def _ParseHashtreeMetadata(self):
        """Parses the hash_algorithm, root_hash, salt from the
           metadata block."""
        ...
        table_len = header[3]
        verity_table = meta_data[META_HEADER_SIZE: META_HEADER_SIZE +
                                 table_len]
        ...
        assert len(table_entries) == 10, \
            "Unexpected verity table size {}".format(len(table_entries))
        assert (int(table_entries[3]) == self.block_size and
                int(table_entries[4]) == self.block_size)
        assert (int(table_entries[5]) * self.block_size ==
                self.filesystem_size and
                int(table_entries[6]) * self.block_size ==
    ...
    def ValidateHashtree(self):
        ...
        # Writes the filesystem section to a temp file; and calls the
        # executable build_verity_tree to construct the hash tree.
        adjusted_partition = common.MakeTempFile(prefix="adjusted_partition")
        with open(adjusted_partition, "wb") as fd:
            self.image.WriteRangeDataToFd(
                self.hashtree_info.filesystem_range, fd)

        generated_verity_tree = common.MakeTempFile(prefix="verity")
        root_hash, salt = BuildVerityTree(
            adjusted_partition, generated_verity_tree)
        ...
        # Reads the generated hash tree and checks if it has the exact same
        # bytes as the one in the sparse image.
    ...
    def Generate(self, image):
        ...
        Raises:
          HashtreeInfoGenerationError: If we fail to generate the exact bytes
              of the hashtree.
        """
        ...
        if not self.ValidateHashtree():
            raise HashtreeInfoGenerationError(
                "Failed to reconstruct the verity tree")
...
def CalculateVbmetaDigest(extracted_dir, avbtool):
    """Calculates the vbmeta digest of the images in the extracted
       target_file"""
    ...
    for name in ("PREBUILT_IMAGES", "RADIO", "IMAGES"):
        ...
        # Create symlink for image files under PREBUILT_IMAGES, RADIO and
        # IMAGES, and put them into one directory.
