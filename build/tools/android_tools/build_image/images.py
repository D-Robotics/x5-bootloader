    ...
    def __init__(self, data, trim=False, pad=False):
        ...
        if partial > 0:
            if trim:
                ...
                raise ValueError(("data for DataImage must be multiple of %d "
                                  "bytes unless trim or pad is specified") %
                                 (self.blocksize,))
        ...
        # When the last block is padded, we always write the whole block even
        # for incremental OTAs. Because otherwise the last block may get
        # skipped if unchanged for an incremental, but would fail the
        # post-install verification if it has non-zero contents in the
        # padding bytes.
        # Bug: 23828506
        if padded:
    ...
    def TotalSha1(self, include_clobbered_blocks=False):
        if not include_clobbered_blocks:
            return self.RangeSha1(self.care_map.
                                  subtract(self.clobbered_blocks))
...
class FileImage(Image):
    ...
    def __init__(self, path, hashtree_info_generator=None):
        ...
        if self._file_size % self.blocksize != 0:
            raise ValueError("Size of file %s must be multiple of %d bytes, "
                             "but is %d" % self.path, self.blocksize,
                             self._file_size)
