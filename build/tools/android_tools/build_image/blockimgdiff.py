...
class Transfer(object):
    ...
    def __str__(self):
        return (str(self.id) + ": <" + str(self.src_ranges) + " " +
                self.style + " to " + str(self.tgt_ranges) + ">")
...
class ImgdiffStats(object):
    ...
    BlockImageDiff. It also logs the ones that cannot use imgdiff,
    with specific reasons. The stats is only meaningful when imgdiff not
    being disabled by the caller of BlockImageDiff. In addition, only files
    with supported types (BlockImageDiff.FileTypeSupportedByImgdiff()) are
    ...
    SKIPPED_NONMONOTONIC = "Not used imgdiff due to having non-monotonic \
ranges"
    ...
    # The list of valid reasons, which will also be the dumped order
    # in a report.
    REASONS = (
        USED_IMGDIFF,
...
class BlockImageDiff(object):
    ...
    provides the following attributes:
       ...
       file_map: a dict that partitions the blocks contained in care_map into
           smaller domains that are useful for doing diffs on. (Typically
           a domain is a file, and the key in file_map is the pathname.)
       ...
       ReadRangeSet(): a function that takes a RangeSet and returns the data
           contained in the image blocks of that RangeSet. The data is
           returned as a list or tuple of strings; concatenating the
    ...
    When creating a BlockImageDiff, the src image may be None, in which case
    the list of transfers produced will never read from the original image.
    """
    ...
    def CanUseImgdiff(self, name, tgt_ranges, src_ranges, large_apk=False):
        ...
        'imgdiff -z' if possible. Because it usually produces significantly
        smaller patches than bsdiff.
        ...
        This is permissible if all of the following conditions hold.
          ...
          - The source and target blocks are monotonic (i.e. the data is
            stored with blocks in increasing order);
          - Both files don't contain shared blocks;
          - Both files have complete lists of blocks;
          - We haven't removed any blocks from the source set.

        If all these conditions are satisfied, concatenating all the blocks in
        the RangeSet in order will produce a valid ZIP file (plus possibly
        extra zeros in the last block). imgdiff is fine with extra zeros at
        the end of the file.
    ...
    def Compute(self, prefix):
        ...
        if common.OPTIONS.cache_size is not None:
            ...
            # Ignore the stash limit and calculate the maximum simultaneously
            # stashed blocks needed.
            _, max_stashed_blocks = self.ReviseStashSize(
                ignore_stash_limit=True)

            # We cannot stash more blocks than the stash limit simultaneously.
            # As a result, some 'diff' commands will be converted to new;
            # leading to an unintended large package. To mitigate this issue,
            # we can carefully choose the transfers for conversion. The number
            # '1024' can be further tweaked here to balance the package size
            # and build time.
    ...
    def WriteTransfers(self, prefix):
        def WriteSplitTransfers(out, style, target_blocks):
            """Limit the size of operand in command 'new' and 'zero'
               to 1024 blocks.

            This prevents the target size of one command from being too large;
            and might help to avoid fsync errors on some devices."""
        ...
        # In BBOTA v3+, it uses the hash of the stashed blocks as the stash
        # slot id. 'stashes' records the map from 'hash' to the ref count.
        # The stash will be freed only if the count decrements to zero.
        stashes = {}
        stashed_blocks = 0
        ...
        for xf in self.transfers:
            ...
            #   new <rangeset>
            #   erase <rangeset>
            #   bsdiff patchstart patchlen srchash tgthash <tgt rangeset>
            #           <src_str>
            #   imgdiff patchstart patchlen srchash tgthash <tgt rangeset>
            #           <src_str>
            ...
            if xf.style == "new":
                ...
                assert tgt_size == WriteSplitTransfers(
                    out, xf.style, xf.tgt_ranges)
                total += tgt_size
            elif xf.style == "move":
                assert xf.tgt_ranges
                assert xf.src_ranges.size() == tgt_size
                if xf.src_ranges != xf.tgt_ranges:
                    # take into account automatic stashing of
                    # overlapping blocks
                ...
                assert WriteSplitTransfers(
                    out, xf.style, to_zero) == to_zero.size()
                total += to_zero.size()
            else:
            ...
            if common.OPTIONS.cache_size is not None:
                # Validation check: abort if we're going to need more stash
                # space than the allowed size (cache_size * threshold).
                # There are two purposes of having a threshold here. a)
                # Part of the cache may have been occupied by some recovery
                # logs. b) It will buy us some time to deal with the oversize
                # issue.
                cache_size = common.OPTIONS.cache_size
                stash_threshold = common.OPTIONS.stash_threshold
                max_allowed = cache_size * stash_threshold
                assert max_stashed_blocks * self.tgt.blocksize <= \
                    max_allowed, 'Stash size %d (%d * %d) exceeds the \
limit %d (%d * %.2f)' % (max_stashed_blocks * self.tgt.blocksize,
                         max_stashed_blocks,
        ...
        # data in the new image; b) will not be touched by dm-verity. Out of
        # those blocks, we erase the ones that won't be used in this update
        # at the beginning of an update. The rest would be erased at the end.
        # This is to work around the eMMC issue observed on some devices,
        # which may otherwise get starving for clean blocks and thus fail
        # the update. (b/28347095)
        ...
        if OPTIONS.cache_size is not None:
            ...
            logger.info(
                "max stashed blocks: %d  (%d bytes), limit: %d bytes "
                "(%.2f%%)\n", max_stashed_blocks, self._max_stashed_size,
                max_allowed, self._max_stashed_size * 100.0 / max_allowed)
        else:
            logger.info(
    ...
    def ReviseStashSize(self, ignore_stash_limit=False):
        ...
        transfer generates. Converts the affected transfers to new if we reach
        the stash limit.
        ...
        # Create the map between a stash and its def/use points. For example,
        # for a given stash of (raw_id, sr), stash_map[raw_id] =
        # (sr, def_cmd, use_cmd).
        for xf in self.transfers:
        ...
        if not ignore_stash_limit:
            # Compute the maximum blocks available for stash based on /cache
            # size and the threshold.
            cache_size = common.OPTIONS.cache_size
            stash_threshold = common.OPTIONS.stash_threshold
            max_allowed_blocks = cache_size * stash_threshold / \
                self.tgt.blocksize
        ...
        # Now go through all the commands. Compute the required stash size on
        # the fly. If a command requires excess stash than available, it
        # deletes the stash by replacing the command that uses the stash
        # with a "new" command instead.
        for xf in self.transfers:
            replaced_cmds = []
            ...
            for stash_raw_id, sr in xf.stash_before:
                ...
                if max_allowed_blocks and \
                        stashed_blocks_after > max_allowed_blocks:
                    # We cannot stash this one for a later command. Find out
                    # the command that will use this stash and replace the
                    # command with "new".
                    use_cmd = stash_map[stash_raw_id][2]
                    replaced_cmds.append(use_cmd)
                    logger.info("%10d  %9s  %s", sr.size(),
                                "explicit", use_cmd)
                else:
            ...
            # "move" and "diff" may introduce implicit stashes in BBOTA v3.
            # Prior to ComputePatches(), they both have the style of "diff".
            if xf.style == "diff":
                ...
                if xf.src_ranges.overlaps(xf.tgt_ranges):
                    if (max_allowed_blocks and
                        ...
                        logger.info("%10d  %9s  %s",
                                    xf.src_ranges.size(), "implicit", xf)
                    else:
                        # The whole source ranges will be stashed for
                        # implicit stashes.
            ...
            for cmd in replaced_cmds:
                # It no longer uses any commands in "use_stash". Remove the
                # def points for all those stashes.
                ...
                # Add up blocks that violates space limit and print total
                # number to screen later.
        ...
        logger.info(
            ...
            "insufficient cache size. Maximum blocks stashed "
            "simultaneously: %d",
        ...
        with open(prefix + ".new.dat", "wb") as new_f:
            for index, xf in enumerate(self.transfers):
                if xf.style == "zero":
                    ...
                    logger.info(
                        "%10d %10d (%6.2f%%) %7s %s %s", tgt_size, tgt_size,
                        100.0, xf.style, xf.tgt_name, str(xf.tgt_ranges))
                    ...
                    # We can't compare src and tgt directly because they may
                    # have the same content but be broken up into blocks
                    # differently, eg:
                    #
                    #    ["he", "llo"]  vs  ["h", "ello"]
                    #
                    ...
                    if xf.src_sha1 == xf.tgt_sha1:
                        # These are identical; we don't need to generate a
                        # patch, just issue copy commands on the device.
                        xf.style = "move"
                        xf.patch_info = None
                        tgt_size = xf.tgt_ranges.size() * self.tgt.blocksize
                        if xf.src_ranges != xf.tgt_ranges:
                            logger.info(
                                ...
                                xf.tgt_name if xf.tgt_name == xf.src_name
                                else (
                                    xf.tgt_name + " (from " +
                                    xf.src_name + ")"),
                        ...
                        if xf.patch_info:
                            # We have already generated the patch (e.g. during
                            # split of large APKs or reduction of stash size)
    ...
    def AssertSha1Good(self):
        ...
        SparseImage.RangeSha1() messed up with the hash calculation in
        multi-thread environment. That specific problem has been fixed
        by protecting the underlying generator function
        'SparseImage._GetRangeData()' with lock.
        """
        for xf in self.transfers:
    ...
    def AssertSequenceGood(self):
        ...
        for xf in self.transfers:
            # Check that the input blocks for this transfer haven't yet
            # been touched.
            ...
            for s, e in x:
                # Source image could be larger. Don't check the blocks that
                # are in the source image only. Since they are not in
                # 'touched', and won't ever be touched.
    ...
    def FindSequenceForTransfers(self):
        """Finds a sequence for the given transfers.
         ...
         The goal is to minimize the violation of order dependencies between
         these transfers, so that fewer blocks are stashed when applying
         the update.
        """
    ...
    def ReverseBackwardEdges(self):
        ...
        For each transfer, make sure it properly stashes the blocks it touches
        and will be used by later transfers. It uses pairs of (stash_raw_id,
        range) to record the blocks to be stashed. 'stash_raw_id' is an id
        that uniquely identifies each pair. Note that for the same range
        (e.g. RangeSet("1-5")), it is possible to have multiple pairs with
        different 'stash_raw_id's. Each 'stash_raw_id' will be consumed by
        one transfer. In BBOTA v3+, identical blocks will be written to the
        same stash slot in WriteTransfers().
        """
    ...
    def FindVertexSequence(self):
        ...
        # the right side of the sequence, built from right to left
        s2 = deque()
        ...
        # Use OrderedDict() instead of set() to preserve the insertion order.
        # Need to use 'sinks[key] = None' to add key into the set. sinks will
        # look like { key1: None, key2: None, ... }.
    ...
    def GenerateDigraph(self):
        ...
        for a in self.transfers:
            ...
            for s, e in a.tgt_ranges:
                for i in range(s, e):
                    if i >= len(source_ranges):
                        break
                    # Add all the Transfers in source_ranges[i] to the
                    # (ordered) set.
            ...
            for b in intersections:
                ...
                # If the blocks written by A are read by B, then B needs to
                # go before A.
                i = a.tgt_ranges.intersect(b.src_ranges)
                if i:
                    if b.src_name == "__ZERO":
                        # the cost of removing source blocks for the __ZERO
                        # domain is (nearly) zero.
                        size = 0
                    else:
                        size = i.size()
    ...
    def ComputePatchesForInputList(self, diff_queue, compress_target):
        ...
        if self.threads > 1:
            logger.info(
                "Computing patches (using %d threads)...", self.threads)
        else:
        ...
        # pattern of the code. The diffing work is done by subprocess.call,
        # which already runs in a separate process (not affected much by the
        # GIL - Global Interpreter Lock). Using multiprocess also requires
        # either a) writing the diff input files in the main process before
        # forking, or b) reopening the image file (SparseImage) in the worker
        # processes. Doing neither of them further improves the performance.
        lock = threading.Lock()
            ...
            while True:
                ...
                if not patch_info:
                    ...
                    try:
                        ...
                        message.append(
                            "Failed to generate %s for %s: tgt=%s, "
                            "src=%s:\n%s" % (
                ...
                if compress_target:
                    ...
                    try:
                        ...
                        compressed_data = (compress_obj.
                                           compress("".join(tgt_data))
                                           + compress_obj.flush())
                        compressed_size = len(compressed_data)
                    except zlib.error as e:
                        message.append(
                            "Failed to compress the data in target range {} "
                            "for {}:\n"
                ...
                with lock:
                    patches[patch_index] = (
                        xf_index, patch_info, compressed_size)
    ...
    def SelectAndConvertDiffTransfersToNew(self, violated_stash_blocks):
        ...
        Since the 'new' data is compressed with deflate, we can select the
        'diff' transfers for conversion by comparing its patch size with the
        size of the compressed data. Ideally, we want to convert the transfers
        with a small size increase, but using a large number of stashed blocks.
        """
        ...
        for xf_index, patch_info, compressed_size in result:
            ...
            logger.info("%s, target size: %d blocks, style: %s, patch size: "
                        "%d, compression_size: %d, ratio %.2f%%",
                        xf.tgt_name, xf.tgt_ranges.size(), diff_style,
                        len(xf.patch_info.content), compressed_size,
                        size_ratio)

            used_stash_blocks = sum(sr.size() for _, sr in xf.use_stash)
            # Convert the transfer to new if the compressed size is smaller or
            # equal. We don't need to maintain the stash_before lists here
            # because the graph will be regenerated later.
            if len(xf.patch_info.content) >= compressed_size:
                # Add the transfer to the candidate list with negative score.
                # And it will be converted later.
                conversion_candidates.append(
                    TransferSizeScore(xf, used_stash_blocks, -1))
            elif used_stash_blocks > 0:
                # This heuristic represents the size increase in the final
                # package to remove per unit of stashed data.
        ...
        # TODO(xunchang), improve the logic to find the transfers to convert,
        # e.g. convert the ones that contribute to the max stash, run
        # ReviseStashSize multiple times etc.
        ...
        for xf, used_stash_blocks, _ in conversion_candidates:
            ...
            # Experiments show that we will get a smaller package size if we
            # remove slightly more stashed blocks than the violated stash
            # blocks.
    ...
    def FindTransfers(self):
        ...
        def AddSplitTransfersWithFixedSizeChunks(tgt_name, src_name,
                                                 tgt_ranges,
            ...
            partition source blocks are too large to be stashed. If a file
            occupies too many blocks, we split it into smaller pieces by
            getting multiple Transfer()s.

            The downside is that after splitting, we may increase the package
            size since the split pieces don't align well. According to our
            experiments, 1/8 of the cache size as the per-piece limit appears
            to be optimal. Compared to the fixed 1024-block limit, it reduces
            the overall package size by 30% for volantis, and 20% for angler
            and bullhead."""
            ...
            while (tgt_ranges.size() > max_blocks_per_transfer and
                ...
                Transfer(tgt_split_name, src_split_name, tgt_first, src_first,
                         self.tgt.RangeSha1(
                             tgt_first), self.src.RangeSha1(src_first),
                         style, by_id)
            ...
            if tgt_ranges.size() or src_ranges.size():
                ...
                tgt_split_name = "%s-%d" % (tgt_name, pieces)
                src_split_name = "%s-%d" % (src_name, pieces)
                Transfer(tgt_split_name, src_split_name, tgt_ranges,
                         src_ranges,
                         self.tgt.RangeSha1(
                             tgt_ranges), self.src.RangeSha1(src_ranges),
                         style, by_id)

        def AddSplitTransfers(tgt_name, src_name, tgt_ranges, src_ranges,
                              style,
                              by_id):
            """Find all the zip files and split the others with a fixed chunk
                size.

            This function will construct a list of zip archives, which will
            later be split by imgdiff to reduce the final patch size. For the
            other files, we will plainly split them based on a fixed chunk
            size with the potential patch size penalty.
            """
            ...
            # Split large APKs with imgdiff, if possible. We're intentionally
            # checking file types one more time (CanUseImgdiff() checks that
            # as well), before calling the costly RangeSha1()s.
            if (self.FileTypeSupportedByImgdiff(tgt_name) and
                ...
                if self.CanUseImgdiff(tgt_name, tgt_ranges, src_ranges, True):
                    large_apks.append(
                        (tgt_name, src_name, tgt_ranges, src_ranges))
                    return
        ...
        def AddTransfer(tgt_name, src_name, tgt_ranges, src_ranges, style,
                        by_id, split=False):
            """Wrapper function for adding a Transfer()."""

            # We specialize diff transfers only (which covers
            # bsdiff/imgdiff/move); otherwise add the Transfer() as is.
            ...
            # Handle .odex files specially to analyze the block-wise
            # difference. If most of the blocks are identical with only
            # few changes (e.g. header), we will patch the changed blocks
            # only. This avoids stashing unchanged blocks while patching.
            # We limit the analysis to files without size changes only. This
            # is to avoid sacrificing the OTA generation cost too much.
            if (tgt_name.split(".")[-1].lower() == 'odex' and
                    tgt_ranges.size() == src_ranges.size()):

                # 0.5 threshold can be further tuned. The tradeoff is: if only
                # very few blocks remain identical, we lose the opportunity to
                # use imgdiff that may have better compression ratio than
                # bsdiff.
                crop_threshold = 0.5
                ...
                for src_block, tgt_block in zip(src_ranges.next_item(),
                    ...
                    if self.src.ReadRangeSet(src_rs) == \
                            self.tgt.ReadRangeSet(tgt_rs):
                ...
                if tgt_changed < tgt_size * crop_threshold:
                    ...
                    # Intentionally change the file extension to avoid being
                    # imgdiff'd as the files are no longer in their original
                    # format.
        ...
        def ParseAndValidateSplitInfo(patch_size, tgt_ranges, src_ranges,
            ...
            # Check that the sizes of all the split pieces add up to the final
            # file size for patch and target.
        ...
        def SplitLargeApks():
            ...
            After the split, the target pieces are continuous and block
            aligned; and the source pieces are mutually exclusive. During
            the split, we also generate and save the image patch between src-X
            & tgt-X. This patch will be valid because the block ranges of src-X
            & tgt-X will always stay the same afterwards; but there's a chance
            we don't use the patch if we convert the "diff" command into "new"
            or "move" later.
            """
            ...
            while True:
                ...
                assert proc.returncode == 0, \
                    ...
                    "{}".format(src_name, tgt_name, imgdiff_output)
        ...
        for tgt_fn, tgt_ranges in sorted(self.tgt.file_map.items()):
            if tgt_fn == "__ZERO":
                ...
                # file and that need to be copied unconditionally to the
                # target.
        ...
        # Sort the split transfers for large apks to generate a determinate
        # package.
        split_large_apks.sort()
        for (tgt_name, src_name, tgt_ranges, src_ranges,
             patch) in split_large_apks:
            transfer_split = Transfer(tgt_name, src_name, tgt_ranges,
                                      src_ranges,
                                      self.tgt.RangeSha1(tgt_ranges),
                                      self.src.RangeSha1(src_ranges),
                                      "diff", self.transfers)
