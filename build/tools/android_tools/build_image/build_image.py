...
def SetUpInDirAndFsConfig(origin_in, prop_dict):
    ...
    When building system.img for all targets, it creates and returns a staged
    dir that combines the contents of /system (i.e. in the given in_dir) and
    root.
...
def CheckHeadroom(ext4fs_output, prop_dict):
    ...
    Headroom is the reserved space on system image (via
    PRODUCT_SYSTEM_HEADROOM), which is useful for devices with low
    disk space that have system image variation between builds. The
    'partition_headroom' in prop_dict is the size in bytes, while the
    numbers in 'ext4fs_output' are for 4K-blocks.
...
def BuildImageMkfs(in_dir, prop_dict, out_file, target_out, fs_config):
    """Builds a pure image for the files under in_dir and writes it to
        out_file.
    ...
    try:
        ...
        # Suppress any errors from GetDiskUsage() to avoid hiding the real
        # errors from common.RunAndCheckOutput().
...
def BuildImage(in_dir, prop_dict, out_file, target_out=None):
    ...
    elif (fs_type.startswith("f2fs") and
          prop_dict.get("f2fs_compress") == "true"):
        fs_spans_partition = False

    # Get a builder for creating an image that's to be verified by Verified
    # Boot, or None if not applicable.
    verity_image_builder = verity_utils.CreateVerityImageBuilder(prop_dict)

    if (prop_dict.get("use_dynamic_partition_size") == "true" and
            "partition_size" not in prop_dict):
        # If partition_size is not defined, use output of `du' + reserved_size.
        # For compressed file system, it's better to use the compressed size
        ...
        if fs_type.startswith("ext"):
            ...
            if "extfs_sparse_flag" in prop_dict:
                sparse_image = True
            fs_dict = GetFilesystemCharacteristics(
                fs_type, out_file, sparse_image)
            os.remove(out_file)
            ...
            if (fs_type.startswith("ext4") and
                    partition_headroom > reserved_size):
                reserved_size = partition_headroom
            if free_size <= reserved_size:
                logger.info(
                    "Not worth reducing image %d <= %d.", free_size,
                    reserved_size)
            else:
                size -= free_size
                size += reserved_size
                if reserved_size == 0:
                    # add .3% margin
                    size = size * 1003 // 1000
                # Use a minimum size, otherwise we will fail to calculate an
                # AVB footer or fail to construct an ext4 image.
    ...
    # Adjust the image size to make room for the hashes if this is to be
    # verified.
    if verity_image_builder:
...
def ImagePropFromGlobalDict(glob_dict, mount_point):
    ...
    if mount_point == "system":
        ...
        # We inherit the selinux policies of /system since we contain some of
        # its files.
        ...
        # Copy the generic fs type first, override with specific one if
        # available.
        copy_prop("fs_type", "fs_type")
        ...
        copy_prop("product_squashfs_disable_4k_align",
                  "squashfs_disable_4k_align")
        ...
        copy_prop("vendor_dlkm_squashfs_compressor_opt",
                  "squashfs_compressor_opt")
        copy_prop("vendor_dlkm_squashfs_block_size", "squashfs_block_size")
        copy_prop("vendor_dlkm_squashfs_disable_4k_align",
                  "squashfs_disable_4k_align")
        ...
        copy_prop("odm_dlkm_squashfs_compressor_opt",
                  "squashfs_compressor_opt")
        copy_prop("odm_dlkm_squashfs_block_size", "squashfs_block_size")
        copy_prop("odm_dlkm_squashfs_disable_4k_align",
                  "squashfs_disable_4k_align")
